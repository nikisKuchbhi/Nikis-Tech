...
class Bunch_Enqueue
{
    ...
    public function bunch_enqueue_scripts()
    {
        ...
        $dark_color = (contra_set($options, 'website_theme') == 'dark') ? true : false;

        $dark_color = (contra_set($_GET, 'color_style') == 'dark') ? true : $dark_color;

        if (contra_set($options, 'color_scheme') == 'color2') : $color = 'css/color2.css'; else: $color = 'css/color1.css';
        endif;
        ...
        if (is_single()) {
            ...
            if ($format == 'gallery') {
                wp_enqueue_script(array( 'contra_jquery-flexslider' ));
            }
            if ($format == 'video' || $format == 'audio') {
                wp_enqueue_script(array( 'contra_jquery-fitvids' ));
            }
        }
        if (is_singular('bunch_projects') || $wp_query->is_posts_page || is_search() || is_tag() || is_category() || is_author() || is_archive()) {
            wp_enqueue_script(array('contra_jquery-flexslider', 'contra_owl.carousel', 'contra_jquery-fitvids'));
        }
        wp_enqueue_script(array('contra_custom_script'));

        wp_add_inline_style('contra-main-style', $this->wp_head());
        $opt = _WSH()->option();
        $h_js = 'if( ajaxurl === undefined ) var ajaxurl = "'.esc_url(admin_url('admin-ajax.php')).'";';
        wp_add_inline_script('contra-main-script', $h_js);
    }

    public function contra_get_font_settings($styles)
    {
        $opt = _WSH()->option();
        $protocol = (is_ssl()) ? 'https' : 'http';
        $font = array();
        ...
        if (contra_set($opt, 'use_custom_font')) {
            if ($h1 = contra_set($opt, 'h1_font_family')) {
                $font[$h1] = urlencode($h1).':400,300,600,700,800';
            }
            if ($h2 = contra_set($opt, 'h2_font_family')) {
                $font[$h2] = urlencode($h2).':400,300,600,700,800';
            }
            if ($h3 = contra_set($opt, 'h3_font_family')) {
                $font[$h3] = urlencode($h3).':400,300,600,700,800';
            }
        }

        if (contra_set($opt, 'body_custom_font')) {
            if ($body = contra_set($opt, 'body_font_family')) {
                $font[$body] = urlencode($body).':400,300,600,700,800';
            }
        }

        if ($font) {
            $styles['bunch_google_custom_font'] = $protocol.'://fonts.googleapis.com/css?family='.implode('|', $font);
        }
    ...
    public function wp_head()
    {
        ...
        $style .= contra_set($opt, 'header_css');
    ...
    public function custom_body_classes( $classes ) {
        ...
        $box_version = contra_set( $options, 'enable_box_version' );
