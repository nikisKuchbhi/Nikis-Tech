...
if (!class_exists('Bunch_Base')) {
    include_once get_template_directory() . '/includes/base.php';
}
add_action('init', 'contra_bunch_theme_init');


if (!function_exists('contra_set')) {
    function contra_set($var, $key, $def = '')
    {
        if (!$var) {
            return false;
        }
...
if (!function_exists('contra_printr')) {
    function contra_printr($data)
    {
        echo '<pre>';
        print_r($data);
        exit;
...
if (!function_exists('contra__font_awesome')) {
    function contra_font_awesome($index)
    {
        $array = array_values($GLOBALS['_font_awesome']);
        if ($font = contra_set($array, $index)) {
            return $font;
        } else {
            return false;
        }
...
if (contra_set($_GET, 'bunch_shortcode_editor_action')) {
    include_once get_template_directory() . '/ resource/shortcode_output.php';
    exit;
}
...
function contra_bunch_theme_init()
{
    global $pagenow;
     ...
     */
    // metaboxes
    $tmpl_mb1  = include CONTRA_ROOT.'includes/vafpress/admin/metabox/meta_boxes.php';
    // * Create instances of Metaboxes
    foreach ($tmpl_mb1 as $tmb) {
        new VP_Metabox($tmb);
    }

    $tmpl_mb1  = include CONTRA_ROOT.'includes/vafpress/admin/taxonomy/taxonomy.php';
    include_once get_template_directory() .  '/vp_new/taxonomy.php' ;
    foreach ($tmpl_mb1 as $tmb) {
        new Bunch_Metabox($tmb);
    }


    // shortocode generators
    $tmpl_sg1  = CONTRA_ROOT.'includes/vafpress/admin/shortcode_generator/shortcodes1.php';
    $tmpl_sg2  = CONTRA_ROOT.'includes/vafpress/admin/shortcode_generator/shortcodes2.php';

    if (is_admin()) {
        include_once get_template_directory() . '/helpers/backup_class.php';
        $backup = new Bunch_Backup_class;

        if (contra_set($_GET, 'page') == 'contra'.'_option') {
            if (contra_set($_GET, 'bunch_dummydata_export')) {
                $backup->export();
            }
        }
    }

    if (function_exists('contra_vc_map')) {
        include_once get_template_directory() .  '/vc_map.php' ;
    }

    // options
    $tmpl_opt  = CONTRA_ROOT.'includes/vafpress/admin/option/option.php';
    ...
    $theme_options = new VP_Option(array(
        ...
        'option_key'            => 'contra'.'_theme_options',                      // options key in db, required
        'page_slug'             => 'contra'.'_option',                      // options page slug, required
        ...
        'page_title'            => esc_html__('Theme Options', 'contra'), // page title
        'menu_label'            => esc_html__('Theme Options', 'contra'), // menu label
    ));
